...
{
    public const string f = "Interaction";
    public const string i = "Inventory";
    public const string e = "Equip";

}

public class ArrayHelper
{
    public static T[] helperAdd<T>(T add, T[] array)
    {
        ArrayList tmp = new();
        foreach (T item in array) tmp.Add(item);
        tmp.Add(add);

        return tmp.ToArray(typeof(T)) as T[];
    }

    public static T[] helperRemove<T>(int index, T[] array)
    {
        ArrayList tmp = new();
        foreach (T item in array) tmp.Add(item);
        tmp.RemoveAt(index);
...
public class SingtonMono<T> : MonoBehaviour where T : Component
{
